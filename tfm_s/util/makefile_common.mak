# For native Windows build, modify BUILD_DIR to change $(SDK_INSTALL_DIR) to 'C:' to avoid errors due to path length limitation.
...
SECURE_DRIVERS_CC13XX_CC26XX_INSTALL_DIR ?= $(SDK_INSTALL_DIR)
DRIVERLIB_LOKI_INSTALL_DIR ?= $(SDK_INSTALL_DIR)
TIDRIVERS_LPF3_INSTALL_DIR ?= $(SDK_INSTALL_DIR)
SECURE_DRIVERS_LPF3_INSTALL_DIR ?= $(SDK_INSTALL_DIR)
HSMDDK_INSTALL_DIR ?= $(SDK_INSTALL_DIR)
TIUTILS_INSTALL_DIR ?= $(SDK_INSTALL_DIR)
MBEDTLS_INSTALL_DIR ?= $(SDK_INSTALL_DIR)
...
ifeq ($(DEVICE_FAMILY), cc26x4)
	...
	SECURE_DRIVERS_INSTALL_DIR := $(SECURE_DRIVERS_CC13XX_CC26XX_INSTALL_DIR)
endif
...
ifeq ($(DEVICE_FAMILY), cc27xx)
	...
	SECURE_DRIVERS_INSTALL_DIR := $(SECURE_DRIVERS_LPF3_INSTALL_DIR)
endif
...
clean:
	...
	@ $(RMDIR) $(call SLASH_FIXUP,$(BUILD_DIR))
	@ $(RMDIR) $(call SLASH_FIXUP,$(NS_BUILD_DIR))
...
configure: generate
	...
	cd $(TFM_ROOT) && $(CMAKE) -S . -B $(BUILD_DIR) \
		...
		-DSECURE_DRIVERS_INSTALL_DIR=$(SECURE_DRIVERS_INSTALL_DIR) \
		-DTIUTILS_INSTALL_DIR=$(TIUTILS_INSTALL_DIR) \
		-DHSMDDK_INSTALL_DIR=$(HSMDDK_INSTALL_DIR) \
...
trim:
	...
	@ $(RMDIR) $(call SLASH_FIXUP,$(TRIM_DIRS))
	@ $(RM) $(call SLASH_FIXUP,$(TRIM_FILES))
