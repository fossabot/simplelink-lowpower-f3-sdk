...
/**
 * @brief Does some actions that should occur right after pkt decrypting and filtering.
 *        Doesn't changes any data in buffer.
 *
 *        Updates neighbor table after decrypting and filtering secured frame.
 *        Removes router from children in case if got first secured pkt from it.
 *        Switches key if needed.
 *
 * @param param buffer that contains already unsecured pkt.
 * @param mac_addr MAC address of neighboring device that has sent this pkt.
 */
void zb_nwk_process_nbt_update_after_unsecur_frame(zb_bufid_t param, zb_uint16_t mac_addr);
