...
typedef ZB_PACKED_PRE struct zb_macsplit_transport_body_s
{
  ...
  /* one byte seems to be enough for macsplit. Optimized it for spinel-based
   * macsplit protocol to keep compatibility in old versions but, indeed can use
   * 1 byte everywhere*/
#ifndef ZB_MACSPLIT_SPINEL
...
typedef struct zb_macsplit_transport_context_s
{
  ...
  zb_uint8_t                       ack_to_tx;               /*!< ack# to be transmitted, INVALID_PKT_NUM if no ack to tx */
  zb_uint8_t                       nack_to_tx;              /*!< nack# to be transmitted, INVALID_PKT_NUM if no nack to tx */
  ...
  zb_uint8_t                       tx_win[4];               /*!< packet ids in tx win. size 4 because packet number size is 2 bits */
